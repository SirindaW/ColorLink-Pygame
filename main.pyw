...
def gen_Map(Map,target):
    ...
    target_Marker = 'A'
    for i in range(0,2*targetPairCount,2):
...
def bfs(Map, start, finish, way_pattern): #normal breath first search
    search_count = 0
    executeCount = 0
    bfs_queue = []
    ...
    while len(bfs_queue)>0:
        ...
        if temp[1] < 9: 
            if Map[temp[0]][temp[1]+1].visited == False: #right
...
def gen_new_map():
    ...
    for i in range(point_count): #mark "ABCDE" in map where point exist 
        Map[Target[i][0]][Target[i][1]].attr = chr(ord(Map_Marker)+(i//2))
    ...
    for i in range(0, len(permbi)):
        ...
        for j in range(0, len(permbi[i])):
            temp_single_target.extend(permbi[i][j])
...
#---------------------------------------------------------------BFS---------------------------------------------------------------------------
    ...
    for i in range(targetPairCount):
...
while running:
    ...
    draw_text(str(memBFS[1]) + ' bytes', (WIDTH/(gridCount*2))*gridRatio[0]+170, HEIGHT-70, red, 30)
    ...
    draw_text(str(memBiBFS[1]) + ' bytes', (WIDTH/(gridCount*2))*gridRatio[1]+170, HEIGHT-70, red, 30)
